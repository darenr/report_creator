...
class D3(Base):
    """
    Embeds a D3Blocks chart into the report.

    The `D3` component allows you to include interactive charts generated by the
    `d3blocks` library. It extracts the HTML content from the d3blocks object
    and embeds it within the report using an iframe to ensure isolation and proper rendering.

    Args:
        d3_obj (Any): The initialized and populated d3blocks object (e.g., from `d3blocks.D3Blocks()`).
            The chart should have been prepared (e.g., `d3.sankey(df, showfig=False)`).
            This component calls `d3_obj.show(return_html=True)` to retrieve the HTML.
        label (Optional[str], optional): An optional label or caption for the chart,
            displayed above it. Inherits from `Base`. Defaults to None.
    """

    def __init__(self, d3_obj: Any, *, label: str | None = None):
        super().__init__(label=label)
        self.d3 = d3_obj
        logger.info(f"D3 component initialized. Label: '{self.label}'")

    @_strip_whitespace
    def to_html(self) -> str:
        """
        Renders the D3 component to an HTML string.

        It retrieves the chart HTML from the d3blocks object and wraps it in an
        iframe. The iframe dimensions are determined from the d3blocks configuration
        if available, defaulting to 800x600.

        Returns:
            str: The HTML representation of the D3 chart component.
        """
        try:
            # Retrieve HTML content from the d3blocks object
            # d3blocks usually returns a full HTML document string with return_html=True
            html_content = self.d3.show(return_html=True)

            # Determine dimensions from d3 config if available
            width = 800
            height = 600
            if hasattr(self.d3, "config") and "figsize" in self.d3.config:
                # figsize is often [width, height] in d3blocks
                figsize = self.d3.config["figsize"]
                if isinstance(figsize, (list, tuple)) and len(figsize) >= 2:
                    # d3blocks figsize is typically [width, height] but let's double check
                    # In some d3blocks charts it might be [width, height]
                    # Default to 800x600 if unsure
                    width, height = figsize

            # Use base64 encoding to embed HTML in iframe src
            b64_html = base64.b64encode(html_content.encode("utf-8")).decode("utf-8")

            iframe_html = (
                f'<iframe src="data:text/html;base64,{b64_html}" '
                f'width="100%" height="{height}px" '
                f'style="border:none; width: 100%; max-width: {width}px;"></iframe>'
            )

        except Exception as e:
            logger.error(f"Error rendering D3 component: {e}\n{traceback.format_exc()}")
            return f"<p><i>Error rendering D3 chart: {html.escape(str(e))}</i></p>"

        html_parts = ['<div class="d3-block"><figure>']
