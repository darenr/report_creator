...
def recolor(color, text):
    regexp = f"(?:{colorcodes[color][True]})(.*?)(?:{colorcodes[color][False]})"
    regexp = regexp.replace("[", r"\[")
    return re.sub(regexp, rf"""<span style="color: {color}">\1</span>""", text)


def bold(text):
    regexp = "(?:{})(.*?)(?:{})".format(colorcodes["bold"][True], colorcodes["bold"][False])
    regexp = regexp.replace("[", r"\[")
...
def underline(text):
    regexp = "(?:{})(.*?)(?:{})".format(
        colorcodes["underline"][True],
        colorcodes["underline"][False],
    )
    regexp = regexp.replace("[", r"\[")
...
def removebackspaces(text):
    ...
    n = 1
    while n > 0:
        text, n = re.subn(backspace_or_eol, "", text, count=1)
    return text
...
if __name__ == "__main__":
    ...
    print(plaintext2html(capture.get()))
    print(repr(capture.get()))
